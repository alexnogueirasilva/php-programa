...
//$queryDemandas = 'SELECT * FROM demanda WHERE id_usr_criador = '.$_SESSION['usuarioID'].' ORDER BY data_criacao ASC';
...
$queryDemandas = 'SELECT d.id, d.mensagem, d.titulo, d.prioridade, d.ordem_servico, d.data_criacao, d.data_fechamento, d.status, d.anexo, u.nome, dep.nome as nome_dep FROM demanda AS d INNER JOIN usuarios AS u ON d.id_usr_destino = u.id AND id_usr_criador = '.$_SESSION['usuarioID'].' and d.status="Fechada" INNER JOIN departamentos AS dep ON u.id_dep = dep.id ORDER BY data_criacao ASC';
